...
const appreciationMessages = [
  "🎉 Amazing! You've cleared all your backlogs!",
  "✨ Outstanding! You're all caught up!",
  "🌟 Fantastic! No pending backlogs!",
  "🚀 Excellent! You've cleared your backlog!",
  "💪 Great effort! Keep it up!",
];

export function Backlogs({ revisions }: BacklogsProps) {
  ...
  if (!Array.isArray(revisions) || sortedRevisions.length === 0) {
    return (
      <div className="relative w-full flex items-center justify-center min-h-64 h-full">
        <Card className="relative p-8 rounded-3xl border border-b-8 border-gray-200 shadow-sm flex flex-col bg-linear-to-br from-green-50 to-emerald-50 z-10 text-center w-full h-full justify-center">
          <p className="text-2xl font-bold text-gray-800 mb-2">
            {appreciationMessages[appreciationIndex]}
          </p>
          <p className="text-sm font-medium text-gray-600">
            Keep maintaining your revision schedule!
          </p>
        </Card>
      </div>
    );
              ...
              >
                ...
                >
                  <p className="text-xs sm:text-sm font-medium text-red-600 mb-2">
                    ⚠️ Due on {formatDate(new Date(rev.next_revision_at))}
                  </p>
                  ...
                  <div className="flex flex-col gap-2 sm:gap-3 mb-3 sm:mb-4 flex-1">
                    <div className="flex justify-between text-xs sm:text-sm gap-2 flex-wrap">
                      ...
                      <span className="text-red-500 font-semibold text-right">
                        Over Due: {formatDate(new Date(rev.next_revision_at))}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-center gap-2 mt-auto">
                    <Link href={`/student/learning/${rev.topic?.id}`}>
                      <Button
                        variant="outline"
                        className="border-2 border-[#ff6b6b] text-[#ff6b6b] rounded-xl sm:rounded-2xl font-bold px-3 sm:px-4 py-2 sm:py-6 text-xs sm:text-base bg-white shadow-none hover:bg-red-50 border-b-4 flex-1 w-full transition-all cursor-pointer"
